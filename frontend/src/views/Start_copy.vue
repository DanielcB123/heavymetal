<template>
	<div class="big-container">
		...
		<div v-if="open" class="flex justify-center fixed z-50 bg-opacity-70 bg-neutral-900 p-5 text-blue-500 w-100 h-full">
			<div class="w-5/6 sm:w-1/3 sm:h-5/12 m-auto rounded-lg bg-slate-400 p-3 bg-opacity-90">
		...
		<!-- MOBILE NAVIGATION -->
		<div class="fixed h-16 bg-gray-300 opacity-80 bottom-0 w-full md:hidden">
			<!-- <button @click="test()" class="z-50 bg-red-500"> here</button> -->
			<div class="w-full h-100 flex justify-end items-center pr-3">
				<button v-on:click="test()" class="z-50 bg-red-500"> here</button>
				<div id="mobileNav" class="flex md:hidden" @click="toggleMobileNav();mobileHamburger()">
					<span></span>
					<span></span>
					<span></span>
				</div>
		...
		<div class="mb-16 fixed h-64 bg-gray-300 opacity-80 bottom-0 w-full md:hidden">
			<!-- <button @click.native="test()" class="z-50 bg-red-500"> here</button> -->
		</div>
...
<!-- <h1 class="w-full bg-red-500 flex justify-center">TEST</h1> -->
		...
		<div class="w-full h-1/2 mt-auto  flex justify-center footer">
			<div class="flex mt-auto justify-between p-5">
				<RouterLink class="rounded-full text-white px-5 py-2 mx-3 join-btn" to="#">JOIN</RouterLink>
				<RouterLink class="rounded-full px-1 text-white px-0 py-2 mx-3 watch-video-btn" to="#">WATCH FULL VIDEO </RouterLink>
				<RouterView />
			</div>
		</div>
...
/* HAMBURGER MENU MOBILE FOTTER NAV */
#nav-icon1, #nav-icon2, #nav-icon3, #mobileNav {
  width: 60px;
  height: 1rem;
  position: relative;
...
#nav-icon1 span, #nav-icon3 span, #mobileNav span {
  display: block;
  position: absolute;
  height: 5px;
  width: 100%;
...
#mobileNav.openBurg span:nth-child(1) {
...
#mobileNav.openBurg span:nth-child(2) {
  width: 0%;
  opacity: 0;
}

#mobileNav.openBurg span:nth-child(3) {
  -webkit-transform: rotate(-45deg);
  -moz-transform: rotate(-45deg);
  -o-transform: rotate(-45deg);
  transform: rotate(-45deg);
